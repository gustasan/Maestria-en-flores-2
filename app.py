...
# La ruta de la carpeta de modelos se obtiene de una variable de entorno
# Esto asegura que la aplicación Streamlit conozca la ubicación correcta de los archivos .pkl
model_dir = os.getenv('IRIS_MODEL_DIR', '/content/drive/MyDrive/DESPLIEGUE MODELO IRIS') # /content/drive/MyDrive/DESPLIEGUE MODELO IRIS será reemplazado por folder_path
...
Asegúrate de que 'best_knn_model.pkl' y 'label_encoder_species.pkl' estén en el directorio especificado.""")
...
<style>
    ...
    .stButton>button {
        ...
        font-weight: bold;
        cursor: pointer;
        transition: background-color 0.3s ease, transform 0.2s ease;h1 {
        color: #4CAF50; /* Verde vibrante para el título principal */
        text-align: center;
        font-size: 2.5em;
        margin-bottom: 0.5em;
...
if st.button('🌿 Predecir especie de la flor'):
    ...
    st.markdown("<div class='species-result'><h2>La especie predicha es: <span style='color:#4CAF50;'>{{}}</span></h2></div>".format(predicted_species[0].capitalize()), unsafe_allow_html=True)
    ...
    if predicted_species[0] in species_info:
        ...
        st.image(info['image'], caption="Imagen de Iris {{}}.".format(predicted_species[0].capitalize()), use_column_width=True)
        # Aseguramos que el texto del color sea blanco para contraste con colores oscuros
        st.markdown("<div class='species-color-box' style='background-color:{{0}};'>Color representativo: {{0}}</div>".format(info['color_hex']), unsafe_allow_html=True)
    else:
...
st.markdown("""
<footer>
    <p>Desarrollado con (corazón) y Streamlit para la predicción de especies de Iris</p>
</footer>
